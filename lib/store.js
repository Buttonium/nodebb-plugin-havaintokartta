...
const { saveReportIfUnchanged } = require('./report-cas');
...
function deserializeReport(value) {
  ...
  return {
    ...
    lat: normalizeCoordinate(value.lat, 'lat'),
    lng: normalizeCoordinate(value.lng, 'lng'),
...
async function saveReport(report, expected) {
  const score = getScore(report);
  const reportKey = KEYS.report(report.id);
  if (expected) {
    if (String(expected.id) !== String(report.id)) throw new Error('Report ID mismatch');
    const saved = { ...report, revision: randomUUID() };
    const indexes = [KEYS.allReports];
    if (saved.creatorUid) indexes.push(KEYS.userReports(saved.creatorUid));
    if (saved.public) indexes.push(KEYS.publicReports);
    await saveReportIfUnchanged(db, reportKey, serializeReport(saved),
      serializeReport(expected), indexes, score);
    return saved;
  }
  // Creation path only. Existing-report mutations must supply their snapshot.


  // Prepare indexes before committing the authoritative record. Never undo a
  // failed save by deleting/restoring data: another writer may have committed.
  // Failed attempts can leave candidate IDs; readers validate the live record.
  await db.sortedSetAdd(KEYS.allReports, score, report.id);
  if (report.creatorUid) {
    await db.sortedSetAdd(KEYS.userReports(report.creatorUid), score, report.id);
  }
  if (report.public) {
    await db.sortedSetAdd(KEYS.publicReports, score, report.id);
  }
  await db.setObject(reportKey, serializeReport(report));
  // Keep public candidate membership on unpublish. Removing it here can race
  // with a concurrent republish; listPublicReports checks current visibility.
...
async function getReports(reportIds) {
  const ids = reportIds || [];
  const reports = [];
  // Bound query size and in-flight work; retain NodeBB's object cache.
  for (let offset = 0; offset < ids.length; offset += 100) {
    const keys = ids.slice(offset, offset + 100).map(KEYS.report);
    const objects = await db.getObjects(keys);
    if (!Array.isArray(objects) || objects.length !== keys.length) throw new Error('Incomplete report batch');
    for (const object of objects) {
      const report = deserializeReport(object);
...
async function listPublicReports() {
  return (await getReports(await listReportIds(KEYS.publicReports)))
...
async function listReportsByUser(uid) {
  return (await getReports(await listReportIds(KEYS.userReports(uid))))
    .filter((report) => String(report.creatorUid) === String(uid));
}
 ...
 *
 * Fetches reports through bounded batches in getReports().
 *
