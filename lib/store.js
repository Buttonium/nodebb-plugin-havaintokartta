...
async function saveReport(report) {
  ...
  // and roll back in reverse order if anything fails. Mirrors the pattern
  // in services.js saveService().
  const done = [];
  ...
  try {
    ...
    if (report.public) {
      ...
      // No rollback entry — this is the last operation and we don't know
      // the previous score to re-add with. Rollback would never reach here.
    }
  } catch (err) {
...
async function deleteReport(reportId) {
  ...
  // Delete associated image file from filesystem (best-effort).
  if (report.images) {
    await deleteReportImage(report.images).catch((err) => {
