...
function parseForumSyncStateToken(token, allowedUrls) {
  ...
  try {
    ...
    // Fail closed: if the allowlist is empty (misconfiguration), reject the
    // token rather than accepting any origin.
...
function buildAuthenticatedRedirectUrl({ returnTo, redirectTo, exchangeCode, state }) {
  ...
  // Echo the original OAuth-style `state` token back to the app so its
  // callback can verify it matches the one-time cookie it set (login-CSRF
  // protection, palvelukartta audit item S4).
  if (state) {
