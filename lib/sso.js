...
async function retrieveAndConsumeExchangeCode(code) {
  ...
  // Atomically claim the code before reading its payload.
  // incrObjectField is atomic in both Redis (HINCRBY) and PostgreSQL
  // (INSERT ... ON CONFLICT ... RETURNING inside a transaction).
  // First caller increments from 0→1; concurrent callers get >1 and
  // are rejected, preserving the single-use guarantee.
...
function parseForumSyncStateToken(token, allowedUrls) {
  ...
  try {
    ...
    // Validate returnTo origin against configured allowed URLs (defense-in-depth).
    // The token is HMAC-signed so tampering is prevented, but a misconfiguration
    // in the app's getAppBaseUrl could produce a returnTo pointing to a wrong host.
    // allowedUrls can be a single string or an array of strings.
