...
async function storeExchangeCode(code, authPayload) {
  ...
  // TTL in seconds; keeps abandoned codes from lingering after 5 minutes.
  await db.expire(EXCHANGE_CODE_KEY(code), Math.ceil(EXCHANGE_CODE_TTL_MS / 1000));
}
...
function buildAuthenticatedRedirectUrl({ returnTo, redirectTo, exchangeCode, state }) {
  ...
  // Echo the state token back so the app can verify it.
  if (state) {
    destinationUrl.searchParams.set('state', state);
  }
...
function createRevocationHash(sig) {
  // Hash of the signature for a fixed-length key.
...
function parseTokenForRevocation(token) {
  ...
  if (typeof token !== 'string' || token.length > 8192 || !APP_SESSION_SECRET) return null;
  const [encodedPayload, signature, ...rest] = token.split('.');
  if (!encodedPayload || !signature || rest.length > 0) {
    return null;
  }
  if (!matchesSignature(signature, createSignature(encodedPayload))) return null;
  try {
    const payload = JSON.parse(Buffer.from(encodedPayload, 'base64url').toString('utf8'));
    // exp must be a finite number.
    if (typeof payload?.exp !== 'number' || !Number.isFinite(payload.exp)) {
      return null;
    }
    const remainingMs = payload.exp - Date.now();
    // Never report successful revocation for less than the token's remaining
    // life. A mismatched issuer TTL must fail visibly instead of reactivating
    // the token when a shortened deny-list entry expires.
    if (remainingMs <= 0 || remainingMs > APP_SESSION_TTL_SECONDS * 1000) return null;
    const remainingSeconds = Math.min(APP_SESSION_TTL_SECONDS, Math.max(1, Math.ceil(remainingMs / 1000)));
...
// Throws 400 on missing/malformed input.
async function verifyTokenClaims(tokenSignature, uid, roles) {
  const sig = String(tokenSignature ?? '').trim();
  if (!sig) {
    throw errors.badRequest('tokenSignature is required.');
  }

  const validUidType = typeof uid === 'number' ||
    (typeof uid === 'string' && /^[1-9][0-9]*$/.test(uid));
  const parsedUid = validUidType ? Number(uid) : NaN;
  if (!Number.isSafeInteger(parsedUid) || parsedUid <= 0) {
    throw errors.badRequest('uid must be a positive integer.');
  }

  if (!Array.isArray(roles) || roles.some((role) => typeof role !== 'string' || !role.trim())) {
    throw errors.badRequest('roles must be an array of non-empty strings.');
  }

  if (await isTokenSignatureRevoked(sig)) {
    return { revoked: true };
  }

  const fields = await user.getUserFields(parsedUid, ['uid', 'banned']);
  if (!fields || fields.uid == null) {
    return { revoked: true };
  }
  if (Number(fields.banned) > 0) {
    return { revoked: true };
  }

  const currentRoles = await resolveRoles(parsedUid);
  const missingRole = roles.find((role) => !currentRoles.includes(role));
  if (missingRole) {
    return { revoked: true };
  }
