...
function initStoragePage() {
		...
		try {
			...
			for (const [subApp, files] of Object.entries(grouped)) {
				...
				$(tableId + ' tbody input[type="checkbox"]').each(function () {
					const row = $(this).closest('tr');
					const rowApp = app === 'ylivieskahub' ? 'ylivieskahub-' + row.data('type') : app;
					if (rowApp === subApp && deletedPaths.has(row.data('path'))) row.remove();
				});
			}

			alert('Deleted ' + totalDeleted + ' file(s).' +
				(totalSkipped > 0 ? ' ' + totalSkipped + ' preserved (referenced or less than 24 hours old).' : '') +
				(totalFailed > 0 ? ' ' + totalFailed + ' failed; files preserved. Refresh the scan before retrying.' : ''));
	...
	$(document).off('click', '.empty-dir-delete-btn').on('click', '.empty-dir-delete-btn', async function () {
		...
		try {
			...
			$(tableId + ' tbody input[type="checkbox"]').each(function () {
				const row = $(this).closest('tr');
				if (deletedPaths.has(row.data('path'))) row.remove();
			});
