...
// MapLibre supports zoom 0–24; values outside it cannot come from the map UI
// and would corrupt fixed-size icon scaling (2^(currentZoom - baseZoom)).
const MAX_BASE_ZOOM = 24;

/** Parse a stored baseZoom value (string or number) into a usable zoom level. */
function parseBaseZoom(value) {
  if (value == null || value === '') return undefined;
  const parsedValue = Number(value);
  if (!Number.isFinite(parsedValue) || parsedValue < 0 || parsedValue > MAX_BASE_ZOOM) {
    return undefined;
  }
  return parsedValue;
}

/** Write-path variant: only real numbers are accepted from API payloads. */
function validateBaseZoom(value) {
  return typeof value === 'number' ? parseBaseZoom(value) : undefined;
}

// Read normalization: drop unusable zoom values so clients fall back to the
// live map zoom instead of computing NaN/Infinity icon sizes.
function sanitizePointBaseZoom(entry) {
  if (entry.baseZoom === undefined) {
    return entry;
  }

  const baseZoom = parseBaseZoom(entry.baseZoom);
  if (baseZoom === undefined) {
    const { baseZoom: _invalidZoom, ...rest } = entry;
    return rest;
  }

  return { ...entry, baseZoom };
}
...
function normalizeHeittopaikat(input) {
  ...
  return input
    .filter((h) => h && typeof h === 'object' && h.type === 'Point' && h.coordinates)
    .map(sanitizePointBaseZoom);
}
...
function normalizeKorit(input) {
  ...
  if (!Array.isArray(input)) return [];
  return input
    .filter((k) => k && typeof k === 'object' && k.type === 'Point' && k.coordinates)
    .map(sanitizePointBaseZoom);
}
...
function serializeService(service) {
  ...
  return {
    ...
    iconScale: typeof service.iconScale === 'number' ? String(service.iconScale) : '',
    fixedSize: toBoolean(service.fixedSize) ? '1' : '0',
    baseZoom: typeof service.baseZoom === 'number' ? String(service.baseZoom) : '',
...
function deserializeService(value) {
  ...
  return {
    ...
    iconScale: value.iconScale ? Number(value.iconScale) : undefined,
    fixedSize: toBoolean(value.fixedSize),
    baseZoom: parseBaseZoom(value.baseZoom),
...
function validateHeittopaikka(value) {
  ...
  const baseZoom = validateBaseZoom(value.baseZoom);
  if (baseZoom !== undefined) result.baseZoom = baseZoom;
  return result;
}
...
function validateKori(value) {
  ...
  if (baseZoom !== undefined) result.baseZoom = baseZoom;
  if (typeof value.text === 'string' && value.text.length <= 20) result.text = value.text;
  if (value.color === 'blue') result.color = value.color;
  return result;
}
...
function validateUrl(value) {
  ...
  // Allow only http(s) — blocks javascript:/data: hrefs.
  if (!['http:', 'https:'].includes(parsedUrl.protocol)) {
    throw errors.badRequest('url must be an http(s) URL.');
  }
  return urlValue;
}
...
async function saveService(service) {
  ...
  // Indexes are candidates, the record is authoritative. Commit it last so
  // an index failure cannot destroy or overwrite an existing service. Do not
  // compensate failures with deletes/restores that can clobber another save.
  await db.sortedSetAdd(KEYS.allServices, score, service.id);
  if (service.category) {
    await db.sortedSetAdd(KEYS.categoryServices(service.category), score, service.id);
  }
  if (service.slug) {
...
async function getServices(serviceIds, { includePrivate = false } = {}) {
  const services = [];
  // Sequential bounded batches avoid one pending lookup per service.
  for (let offset = 0; offset < serviceIds.length; offset += 100) {
    const keys = serviceIds.slice(offset, offset + 100).map(KEYS.service);
    const objects = await db.getObjects(keys);
    if (!Array.isArray(objects) || objects.length !== keys.length) throw new Error('Incomplete service batch');
    for (const object of objects) {
      const service = deserializeService(object);
...
async function listServicesByCategory(category, { includePrivate = false } = {}) {
  return (await getServices(await listServiceIds(KEYS.categoryServices(category)), { includePrivate }))
...
async function createService(payload, actorUid) {
  ...
  const service = {
    ...
    iconScale: (typeof payload?.iconScale === 'number' && payload.iconScale >= 0.5 && payload.iconScale <= 2) ? payload.iconScale : undefined,
    fixedSize: toBoolean(payload?.fixedSize),
    baseZoom: validateBaseZoom(payload?.baseZoom),
...
async function updateService(serviceId, payload, actorUid) {
  ...
  if (payload?.iconScale !== undefined) {
    updated.iconScale = (typeof payload.iconScale === 'number' && payload.iconScale >= 0.5 && payload.iconScale <= 2) ? payload.iconScale : undefined;
  }
  if (payload?.fixedSize !== undefined) {
    updated.fixedSize = toBoolean(payload.fixedSize);
  }
  if (payload?.baseZoom !== undefined) {
    updated.baseZoom = validateBaseZoom(payload.baseZoom);
  }
  ...
  // Index cleanups run only after the save succeeded, so a failure in between
  // cannot leave the previous slug/category unresolvable.
  if (updated.slug !== existing.slug && existing.slug) {
    await db.delete(KEYS.slugIndex(existing.slug)).catch((err) => {
      console.error('[palvelukartta] Failed to remove old slug index:', err?.message || err);
    });
  }
  if (updated.category !== existing.category && existing.category) {
    await db.sortedSetRemove(KEYS.categoryServices(existing.category), serviceId).catch((err) => {
      console.error('[palvelukartta] Failed to remove old category index:', err?.message || err);
    });
  }
...
async function searchServices({ query = '', categories = [], includePrivate = false } = {}) {
  ...
  if (!trimmedQuery && categorySet.size > 0) {
    ...
    return (await getServices([...ids], { includePrivate }))
      .filter((service) => categorySet.has(service.category));
  }
