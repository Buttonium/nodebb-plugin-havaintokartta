...
function normalizeCoordinate(value, type) {
  ...
  // Axis bounds mirror the app-side validation. Stored legacy values that
  // are out of range read back as null so no consumer sees garbage.
  if (type === 'lat' && (parsedValue < -90 || parsedValue > 90)) {
    return null;
  }
  if (type === 'lng' && (parsedValue < -180 || parsedValue > 180)) {
    return null;
  }
