...
// Canonical report images are a JSON array string; legacy values are a bare
// URL. Anything else is unreadable — including JSON scalars such as a quoted
// URL — so the lookup fails closed instead of letting those files look
// unreferenced.
function assertReportImageReferences(report) {
	if (typeof report.images !== 'string') return;

	const trimmed = report.images.trim();
	...
	try {
		...
		// Handled below — malformed JSON is treated like a non-array value.
	}
	if (!Array.isArray(parsed)) {
		throw new Error(`Invalid stored report image references for report ${report.id ?? '(unknown id)'}`);
	}
}
...
async function getReferencedReportPaths(config, prefix = config.uploadsUrlPrefix) {
	...
	for (const report of reports) {
		if (report && report.images) {
			assertReportImageReferences(report);
	...
	for (const service of services) {
		if (service && service.images) {
			...
			if (!Array.isArray(images)) {
				throw new Error(`Invalid stored service image references for service ${service.id ?? '(unknown id)'}`);
			}
