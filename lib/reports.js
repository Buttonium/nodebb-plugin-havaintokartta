 ...
 * write is gated by an optimistic re-read because saveReport is a blind
 * full-object replace (a concurrent review must yield 409, not a clobber).
 */
async function updateReport(reportId, payload) {
  ...
  if (payload?.images != null) {
    ...
    const expectedPrefix = `/${String(uploadsUrlPrefix ?? '').trim().replace(/^\/+/, '').replace(/\/+$/, '')}`;
  ...
  await store.saveReport(updatedReport);

  // If the stage progressed after our write, the edit is not in effect.
  const afterSave = await store.getReport(normalizedReportId);
  if (!afterSave || afterSave.stage !== 1) {
    throw errors.conflict('Report status changed while saving. Your edit was not applied.');
  }
...
// References this plugin's report uploads dir; accepts absolute and relative forms.
