...
function buildTopicContent(payload) {
  return [
    ...
    '',
    `Kuva mukana: ${payload.images ? 'kyllä' : 'ei'}`,
  ].join('\n');
}
...
function normalizeCreatePayload(payload) {
  return {
    ...
    images: sanitizeSingleLineText(payload?.images, 1000) || null,
  };
}
...
async function reviewReport(reportId, payload) {
  ...
  // If operator chose not to publish the image, delete it from disk and clear the reference.
  // This ensures the image cannot be accessed via direct URL after review.
