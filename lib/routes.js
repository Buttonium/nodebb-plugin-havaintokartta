...
function errorHandler(err, req, res, next) {
  ...
  if (!res.headersSent) {
    // Hide internal details on 5xx; the full message is already logged above.
    if (status >= 500) {
...
function mount(parentRouter) {
  ...
  router.post('/reports/delete-image', apiWriteLimiter(), requireApiKey, asyncRoute(controllers.deleteReportImage));
  ...
  router.post('/reports/:id/update', apiWriteLimiter(), requireApiKey, asyncRoute(controllers.updateReport));
