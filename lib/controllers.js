...
function isOriginAllowed(origin) {
  const { appUrl, ylivieskahubUrl, palvelukarttaUrl, extraAllowedOrigins } = getConfig();
  const allowedOrigins = [appUrl, ylivieskahubUrl, palvelukarttaUrl, ...(extraAllowedOrigins || [])]
    .map((u) => String(u ?? '').trim())
    .filter(Boolean);
...
async function deleteReportImage(req, res) {
  const body = getBody(req);
  const imageUrl = String(body?.imageUrl ?? '').trim();
  const deleted = await upload.deleteReportImage(imageUrl);
  res.json({ deleted });
}
...
async function syncAuth(req, res) {
  const { appUrl, ylivieskahubUrl, palvelukarttaUrl, extraAllowedOrigins } = getConfig();
  const allowedUrls = [appUrl, ylivieskahubUrl, palvelukarttaUrl, ...(extraAllowedOrigins || [])]
    .map((u) => String(u ?? '').trim())
    .filter(Boolean);
...
async function updateReport(req, res) {
  const report = await reports.updateReport(req.params.id, {
    ...getBody(req),
    actorUid: getActorUid(req),
  });
  res.json({ report });
...
/** Revoke a session token by deny-listing its signature. */
...
async function verifyToken(req, res) {
  const body = getBody(req);
  const result = await sso.verifyTokenClaims(body.tokenSignature, body.uid, body.roles);
  res.json(result);
}
