...
async function uploadImage(req, directory, urlPrefix) {
  ...
  // Always emit an absolute path so every consumer (app parsers, delete
  // prefix checks) sees a `/`-prefixed reference regardless of how the
  // upload_url config is written.
  const urlPrefixSegment = String(urlPrefix ?? '').trim().replace(/^\/+/, '').replace(/\/+$/, '');
  const relativePath = `/${urlPrefixSegment}/${dateSegment}/${fileName}`;
...
async function deleteImage(imageUrl, directory, urlPrefix) {
  ...
  // Stored references are normally absolute URLs, but legacy/relative paths
  // (e.g. when the NodeBB base URL was not configured at upload time) must
  // also be deletable. Prefix and traversal checks below still apply.
  let pathname;
  if (!/^[a-z][\w+.-]*:/i.test(trimmedUrl)) {
    pathname = `${trimmedUrl.startsWith('/') ? '' : '/'}${trimmedUrl.split(/[?#]/, 1)[0]}`
      .replace(/\\/g, '/');
  } else {
    try {
  ...
  // Require a separator after the namespace so look-alike prefixes such as
  // `reports-extra/...` cannot alias files inside the real uploads directory.
  ...
  if (!relativePath) {
    console.warn('[havaintokartta] deleteImage: URL does not reference a file:', imageUrl);
    return false;
  }
